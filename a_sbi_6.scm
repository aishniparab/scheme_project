    ...
    (define var (car stmt))
         (cond
            ...
            [(eqv? var '/)
               (/ (handle-expr (cadr stmt)) (handle-expr (caddr stmt)))]
            [(eqv? var '%)
               (- (handle-expr (cadr stmt)) (* (truncate (/ (handle-expr (cadr stmt)) (handle-expr (caddr stmt)))) (handle-expr (caddr stmt))))]
            [(eqv? var 'sqrt)
               (sqrt (handle-expr (cdr stmt)))]
            [(eqv? var '^)
               (expt (handle-expr (cadr stmt)) (handle-expr (caddr stmt)))]
            [(eqv? var 'log)
...
(define (handle-expr stmt)
   (if(list? stmt)
      ;if stmt is a list then call expr-list
      (expr-list stmt)
      ;if stmt is not a list then call expr-unlist
      (expr-unlist stmt)
   )
)
...
(define (handle-let line)
    ;when array
    ;(display "in handle-let line, car is:")
    ;(display (car line))
    ;(display "cadr line is:")
    ;(display (cadr line))
...
(define (process-opt line)
   ...
   ;I NEED TO MAKE SURE I PASS IN A LINE OF THE FOLLOWING FORM:
   ;(OPT EXPR...)
   ;NOT LIKE:
   ;OPT EXPR
   ;((OPT EXPR))
