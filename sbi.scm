...
(display relop-expr)
(display (cadr relop-expr))
(display (cddr relop-expr))
   ...
   (cond
       ...
       [(eqv? relop '>=)
         (>= l-expr r-expr)]
       [(eqv? relop '<=) ;(display (<= l-expr r-expr))
         (<= l-expr r-expr)]
   )
)
...
(define (input-opt line)
      (put-variable! 'inputcount (-(get-variable 'inputcount) 1))
      (let ((value (read)))
      (put-variable! (cadr line) (+ value 0.0))
      )
)
...
(display line)
;(display (cadr line))
  ;(when (> (length (car prgm)) 0)
  ;  (put-variable! 'inputcount (+(get-variable 'inputcount) 1))
  ;  (define value (read))
  ;  (put-variable! (cadr line) (+ value 0.0))
  (when (> (get-variable 'inputcount) 0)
      (input-opt line)
      (handle-input (cdr line))
  )
)
...
(define (process-program prgm)
   ...
   ;(when not (null? (cdr prgm)) 
        ...
        (when (symbol? (cadr (car prgm)))
           (if (= (length (car prgm)) 2) (process-opt (cddr(car prgm)))
